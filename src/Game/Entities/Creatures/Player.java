...
public class Player extends CreatureBase {
	...
	private boolean summonCompanion = false;
	...
	public void tick() {
		...
		//Summon
		if(handler.getKeyManager().callbutt)
			checkCompanionItem();
	...
	public void checkCompanionItem() {
		if(inventory.getInventoryItems().contains(Item.items[6])) {
			summonCompanion = true;
			Item.items[6].setCount(0);
		}
	}
	
	
	
	public boolean getSummonCompanion() {
		return summonCompanion;
}
